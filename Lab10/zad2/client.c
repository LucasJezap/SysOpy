...
int main(int argc, char *argv[]) {
    ...
    send_msg(server_sock_id,LOGIN_REQ,buf,nickname);
        ...
        while (!0) {
            ...
            if (msg->mtype == GAME_WAIT) {
                ...
                if (sign == 'X') {
                    ...
                    send_msg(server_sock_id,GAME_MOVE,input,nickname);
                }
                else 
                ...
                while(!0) {
                    ...
                    if (msg->mtype == GAME_MOVE) {
                        ...
                        send_msg(server_sock_id,LOGOUT,NULL,nickname);
                        printf("Logging out from the server...\n");
                        break;
                    }
                    else if (msg->mtype == PING) {
                        YELLOW();
                        send_msg(server_sock_id,PING,NULL,nickname);
                    }
                }
                break;
            }
...
int check_nick(char *nickname) {
    FILE *file = fopen("nicks","r");
    char line[50];
    while (fgets(line,sizeof(line),file)) {
        int i=0;
        while (line[i] != ' ') {
            if (nickname[i] != line[i])
                break;
            i++;
        }
        if (line[i] == ' ')
...
void server_connect() {
    if (check_nick(nickname) == 0) {
        printf("Server rejected login. Reason: user_exists\n");
        exit(0);
    }
    char comm[50];
    sprintf(comm,"echo '%s ' >> nicks",nickname);
    system(comm);
    if (is_local) {
        ...
        if ((server_sock_id = socket(AF_UNIX,SOCK_DGRAM,0)) == -1) {
        ...
        strcpy(sock_client.sun_path,nickname);

        if (bind(server_sock_id, (struct sockaddr *) &sock_client, sizeof(sock_client)) == -1) {
            print_error("Error while binding to the socket!");
            return;
        }
        ...
        if ((server_sock_id = socket(AF_INET,SOCK_DGRAM,0)) == -1) {
            print_error("Error while connecting to socket!");
            return;
        }
        ...
        sock_client.sin_addr.s_addr = inet_addr(address);
        sock_client.sin_port = 0;
...
void server_disconnect() {
    ...
    if (is_local && unlink(nickname) == -1) {
        print_error("Error while unlinking client socket!");
    }
